...
// Code adapted from karl malbrain at:
// https://code.google.com/archive/p/compression-code/
...
long int vitter(int argc, char **argv)
{
    ...
    //temporary file since we just want to see how large the resulting file is
    if( argc > 3 )
    ...
    int status = 0;
    status = remove(argv[3]);

    if( status != 0 )
    {
      printf("Unable to delete the Vitter file\n");
      perror("Error");
    }    
